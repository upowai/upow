...
class Database:
    ...
    async def remove_blocks(self, block_no: int):
        """
        Remove all blocks with id >= block_no and restore the full chain state.

        Delegates to perform_reorg() which handles:
          - Regular UTXO restoration
          - All governance table restoration (inode, validator, delegate, ballot)
          - Orphaned transaction re-queuing into the mempool
          - emission_details cleanup
          - Manager.difficulty cache invalidation
          - clear_pending_transactions() to purge invalid mempool entries
        """
        from .reorg import perform_reorg
        await perform_reorg(block_no)
