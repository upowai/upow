...
NODE_URL = 'http://127.0.0.1:3006'


def get_address_info(address: str):
    request = requests.get(f'{NODE_URL}/get_address_info',
...
def create_transaction(private_keys, receiving_address, amount, message: bytes = None, send_back_address=None):
    ...
    if transaction_amount < amount:
        raise Exception(f"Consolidate outputs: send {transaction_amount} upow to yourself")
