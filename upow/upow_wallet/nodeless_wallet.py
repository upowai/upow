...
from upow.upow_wallet.utils import string_to_bytes
...
def get_address_info(address: str):
    ...
    pending_spent_outputs = [
        tuple(output) for output in result["pending_spent_outputs"]
    ]
...
def create_transaction(
    ...
    for private_key in private_keys:
        ...
        if (
            sum(
                input.amount
                for input in sorted(
                    inputs, key=lambda item: item.amount, reverse=False
                )[:255]
            )
            >= amount
        ):
    ...
    if transaction_amount < amount:
        raise Exception(
            f"Consolidate outputs: send {transaction_amount} upow to yourself"
        )
...
async def main():
    ...
    parser.add_argument(
        ...
        choices=["createwallet", "send", "balance"],
    )
    ...
    db = pickledb.load(f"{dir_path}/upow_wallet.json", True)
    ...
    if command == "createwallet":
        ...
        parser.add_argument(
            "-d", metavar="amount", type=str, dest="amount", required=True
        )
        parser.add_argument(
            "-m", metavar="message", type=str, dest="message", required=False
        )
        ...
        tx = create_transaction(
            db.get("private_keys"), receiver, amount, string_to_bytes(message)
        )
